package com.hmt.graduate.paper.controller.user;

import com.hmt.graduate.paper.constant.IStatusMessage;
import com.hmt.graduate.paper.controller.BaseController;
import com.hmt.graduate.paper.service.UserService;
import com.hmt.graduate.paper.utils.*;
...
import com.hmt.graduate.paper.entity.ResponseResult;
...
//	private ParamProperties paramProperties;
	...
	// 与前端cryptoUtil.js里面的key和iv值保持一致
	private static String aesKey = ".admin.wyait.com";
	private static String aesIv = "abcdef1234567890";
	...
	public ResponseResult login(UserDTO user) {
		...
		try {
			// 1.1 aes解密密码
			String pwd = AesEncryptUtil.decryptAes(user.getPassword(), aesKey,
					aesIv);
			logger.debug("用户登录，用户密码AES解密！pwd={}", pwd);
			// 1.2 校验密码规则
			if (!ValidateUtil.isSimplePassword(pwd)) {
				responseResult.setMessage("用户名或密码有误，请您重新填写");
				logger.debug("用户登录，用户名或密码有误，请您重新填写!结果=responseResult:{}",
						responseResult);
				return responseResult;
			}
			// 1、 封装用户名、密码、是否记住我到token令牌对象 [去除支持记住我]
			AuthenticationToken token = new UsernamePasswordToken(
					user.getMobile(), DigestUtils.md5Hex(pwd));
			// 2、 Subject调用login
		 ...
		 * .getCache("passwordRetryCache"); if (null != passwordRetryCache) {
		 * int retryNum = (passwordRetryCache.get(existUser.getMobile()) == null
		 * ? 0 : passwordRetryCache.get(existUser.getMobile())).intValue();
		 * logger.debug("输错次数：" + retryNum); if (retryNum > 0 && retryNum < 6) {
		 * responseResult.setMessage("用户名或密码错误" + retryNum + "次,再输错" + (6 -
		 * retryNum) + "次账号将锁定"); } }
		 */
		/*
		 * try { //3 记住我，设置redis中用户数据的有效时间 if(rememberMe){ Cookie[]
		 * cookies=request.getCookies(); String sessionCookie=""; if (cookies !=
		 * null && cookies.length > 0) { //遍历所有Cookie for(Cookie cookie:
		 * cookies) { //找到name为JSESSIONID的Cookie【用户cookie】 if
		 * (cookie.getName().equals("JSESSIONID")) {
		 * //使用URLDecode.decode()解码,防止中文乱码 // sessionCookie =
		 * URLDecoder.decode(cookie.getValue(),"utf-8");
		 * sessionCookie=cookie.getValue(); logger.debug("用户登录，user=" +
		 * user.getMobile() + ",记住我，用户cookie:{}",sessionCookie);
		 * 
		 * if(StringUtils.isNotBlank(sessionCookie)){
		 * redisUtil.expire(paramProperties.getShiroRedisSessionPrefix()+
		 * sessionCookie,paramProperties.getRememberMeExpire());
		 * logger.debug("用户登录，user=" + user.getMobile() +
		 * ",记住我，设置redis中session用户数据过期时间为3天！"); break; } } } } long num =
		 * redisUtil.getExpire(paramProperties.getShiroRedisSessionPrefix()+
		 * sessionCookie); logger.debug("用户登录，redis中用户有效时间:{}",num); } }catch
		 * (Exception e){ logger.error("用户登录，记住我，操作失败！user=" + user.getMobile(),
		 * e); responseResult.setMessage("记住我，登录失败"); }
		 */
